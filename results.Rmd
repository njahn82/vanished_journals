...
my_jn_timeline %>%
  ...
  geom_linerange() +
  labs(y = "Publication history", x = "Vanished OA Journals") +
...
table_1 <- my_jn_timeline %>% 
  group_by(end_year) %>% 
  summarise(n = n(), median_dur = median(duration), sd_dur = sd(duration)) %>%
  ungroup() %>%
  mutate(end_year = as.character(end_year)) %>% 
  t() %>% 
  as_tibble() %>%
  row_to_names(row_number = 1)
Total <- c(nrow(my_jn_timeline), median(my_jn_timeline$duration), sd(my_jn_timeline$duration))
duration_table <- cbind(table_1, Total)
...
```{r, fig.cap="Life-span distribution of vanished fully open access journals across subject domains, in years. Each point represent a journal and its life-span."}
  ...
  labs(x = "Subject domain", y = "Life-span in years") +
...
wb_df <- readxl::read_xls("data/CLASS.xls", skip = 4) %>%
  select(Economy, Code, Region, income_group =  `Income group`) %>%
...
p +
 ...
 # scale_fill_brewer("Journal Subject", palette = "Dark2") +
  scale_fill_manual("Journal Subject", values = c("#65BADA", "#00A757", "#E5BA52", "#D86F27")) +
  labs(x = NULL, y = NULL) +
  coord_equal() +
  theme_minimal() +
...
ggplot(my_jn_online_availability, aes(x = lag_archiving)) +
  ...
  labs(y = "Journals", x = " Lag between last journal activity and vanishing (in years)")
```
