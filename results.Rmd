...
my_jn_timeline %>%
  ...
  mutate(language_cat = case_when(
    ...
    grepl("English", `Language(s)`, fixed = TRUE) ~ "Multiple languages including English",
  ...
  summarise(n = n(),
            ...
            sd = sd(duration)) %>%
  mutate(prop = n / sum(n) * 100) %>%
  select(`Journal Language(s)` = 1, `Vanished Journals` = n, `Proportion (in %)` = prop, `Mean Age (in years)` = mean_dur, SD = sd) %>%
  knitr::kable(digits = 2)
```
